...
- `src/layout/engine/patch_error.rs` 定义 `IncrementalLayoutError` 和
  `LayoutLookupError`，`src/layout/mod.rs` 导出 public checked caller 所需类型。两者都实现
  `Error`；只有 `IncrementalLayoutError` 这类 composite wrapper variant 通过
