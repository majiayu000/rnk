...
rebuild typed error 仍属于 GH-60；raw `Patch` topology、whole-frame terminal transaction、
rollback/rebuild/fallback 也不在 GH-59 扩张。成功 subtree map 清理属于 GH-59。
`LayoutSnapshot` parity/benchmark 属于 GH-61。

## Codebase Context

以下 table 是最初 stacked spec 阶段在 `spec/GH58-text-flow`
`6e6e58932a009ab5c205a9227f996b1d4f604b35`（当时 production 等于
`origin/main` `e4a89ae128533270d28d768d49977a05a389a582`）记录的 historical root-cause anchors；
它解释问题来源，不再充当 current path/line ownership manifest。当前 corrective
implementation 基于 `origin/main` `9126bf2bf5073b8243d3d42b0d93421094df12ec`，实际
base-to-worktree paths 以第 5 节 manifest 为准。
...
| Runtime compatibility publication | `src/renderer/pipeline.rs:34`, `src/runtime/context.rs:428`, `src/hooks/use_measure.rs:74` | renderer 从 engine 取 current NodeKey/layout 后建立单值 string alias；runtime 无法表达 same raw key 的多个 parent scope | engine 保留 composite view，renderer/runtime 还必须发布 scoped/raw candidates 并让旧 helper 在歧义时 fail loudly |
...
- direct VNode 的 opaque token 没有原值可做二次 equality；同 parent 相同 token 按
  `ReconcilePlanError::DuplicateSiblingKey { key_kind: IdentityKeyKind::Opaque, .. }` 拒绝。
- exact-string collision 使用 crate-private token derivation seam：production
  `plan_diff` 注入与 `NodeKey::with_key` 相同的 `compatibility_token_for_exact(&str) -> u64`；
...
ReconcilePlanError
  ...
  -> RuntimeContext checked measurement publication/lookup
  -> App::render_frame -> io::Error(source = DynamicFrameError)

ReconcilePlanError / LayoutLookupError / PatchError
  -> DirectPatchError::{Identity, Lookup, Patch}
  -> LayoutEngine::try_apply_patches_checked
```

- `src/layout/engine/patch_error.rs` 定义 `IncrementalLayoutError`、`LayoutLookupError` 和
  `#[non_exhaustive] DirectPatchError`，`src/layout/mod.rs` 导出 public checked caller 所需类型。
  三者都实现 `Error`；`IncrementalLayoutError` / `DirectPatchError` composite wrapper 通过
  `Error::source` 暴露内层 identity/TextFlow cause，leaf `LayoutLookupError` 没有 nested
  source。`DynamicFrameError` 再把 incremental/text/lookup leaf 作为自己的 source。
- 既有 `PatchFailure` 必须保持 GH-58 的六个 variants，`PatchError` 与
  `try_apply_patches -> Result<bool, PatchError>` 签名不变；旧下游的无 wildcard 穷举 match
  必须继续编译。`try_apply_patches_checked` 是唯一 direct raw `Patch` batch 实现 core；旧
  `try_apply_patches` / `apply_patches` 只委托它，遇 legacy error 原样返回/维持 bool contract，
  遇 identity 或 scoped lookup cause 则 fail loudly。typed caller 使用 `DirectPatchError`。
...
- `src/renderer/app.rs` 只调用 checked pipeline；pipeline 只有在 layout identity、compatibility
  projection 与 render 全部成功后，才把 scoped layouts、composite layouts、raw candidates
  与 string aliases 交给 `RuntimeContext`，并更新 previous VNode。identity 失败不得发布新
  measurement state；App 的 terminal frame commit 边界与 whole-frame transaction 留给 GH-60。
- 旧 `render_dynamic_frame` / GH-58 TextFlow-only try wrapper 保持签名，identity error
  时 fail loudly。Taffy apply/rebuild failure variants 与恢复策略继续由 GH-60 contract 拥有。
...
collision 时返回 `CompositeIdentityCollision`；旧 all-layouts wrapper fail loudly，正常 same-key/
...
runtime measurement compatibility 不能把 engine 已保留的 parent scope 再压回单值 alias。
`RuntimeContext` 必须同时保存本次成功 publication 的 ElementId measurements、exact
`ScopedNodeIdentity` measurements、engine composite projections、raw
`SiblingIdentity -> Vec<ScopedNodeIdentity>` candidates 和
`String -> Vec<(ScopedNodeIdentity, SiblingIdentity)>` aliases。现有 helper 签名保持不变，
checked lookup 使用现有 `try_get_measurement_by_node_key_dims`、
`try_get_measurement_by_key_dims` 与 `try_resolve_measurement_key_alias`：0 个 semantic
candidate 返回 `Ok(None)`，全局唯一 candidate 返回该 exact scoped measurement/projection，
多个 parent-scoped candidates 返回 `LayoutLookupError` 的 typed ambiguity。对应 legacy
`get_measurement_by_node_key_dims`、`get_measurement_by_key_dims`、
`resolve_measurement_key_alias` 以及 `measure_element_by_key` 在 ambiguity 时 fail loudly。
这不改变 engine composite projection：composite key 仍可无歧义精确读取。

`set_measure_layouts_with_scoped_keys` 是一次成功 frame publication 的单一替换点；它必须在
新 snapshot 已完整收集后一次性替换上述所有 views。unique -> ambiguous -> unique 连续帧
必须清除前一帧 ElementId、scoped/composite/raw candidate 和 string alias，未知 raw/string 返回
`None`；layout identity validation、compatibility projection 或 render 失败时 pipeline 不调用
该 publication，runtime measurement snapshot 保持原值。这里的“原子”只约束 runtime map
replacement 的成功 publication，不声明 GH-60 的 raw `Patch` 重构、Taffy rollback、rebuild、
fallback 或 whole-frame terminal transaction。

每个 `ParentPlan` 先由 `ReconcilePlan::validate_final_orders` 做纯 validation，再由
`LayoutEngine::preflight_reconcile_plan` 解析现有 NodeId；这两步都不触碰 Taffy：
...
7. 成功 read-back 后，以 target planned tree/validated `ReconcilePlan` 和已解析 NodeId 构造新的 scoped
...
### 5. 实际 planned paths、文件拆分与 GH-58 协作

以下 manifest 必须与 trusted base `9126bf2` 到最终 clean exact head 的
production/test/checker diff 逐项一致：

- public/reconciler：`src/core/vnode.rs`；`src/reconciler/mod.rs`、
  `src/reconciler/error.rs`、`src/reconciler/identity.rs`、`src/reconciler/plan.rs`、
  `src/reconciler/diff.rs`。planner/adapter tests 与 controlled-token collision tests 直接位于
  `diff.rs`；identity/error/plan 分离为各自实际模块。
- layout：`src/layout/mod.rs`、`src/layout/engine.rs`、
  ...
  `identity_index.rs` owns checked raw/composite projection，`incremental.rs` owns scoped
  Element/VNode materialization/apply，`incremental_order.rs` owns final-order preflight，
  `patch_error.rs` owns checked incremental/lookup errors；context sync 与 TextFlow bridge 的
  edits only adapt existing GH-58 state to scoped identities。All listed Rust files remain below 800 lines。
- `patching.rs` 的 GH-59 diff only adds scoped identity/address resolution、direct-patch identity
  preflight 与 exact cleanup/postconditions。它不改变 public raw `Patch` enum topology，也不
  authorize GH-60 whole-frame terminal transaction、rollback/rebuild/fallback work。
- dynamic/runtime：`src/renderer/mod.rs`、`src/renderer/error.rs`、
  `src/renderer/pipeline.rs`、`src/renderer/app.rs`、`src/runtime/context.rs`、
  `src/hooks/use_measure.rs`。pipeline only publishes a fully successful scoped/composite/raw/string
  measurement snapshot；hook file includes the fail-loud ambiguity docs and regression。
- private critical coverage/error-matrix evidence：`src/reconciler/identity/tests.rs`、
  `src/reconciler/plan/tests.rs`、`src/layout/engine/incremental/tests.rs`、
  `src/layout/engine/incremental_order/tests.rs`。这些文件只由对应 production module 的
  `#[cfg(test)] mod tests` 接入；checker 按 `tests` path component 将其排除在 production
  changed-executable-line >=80% denominator 之外，但其执行必须覆盖下列四个 critical production
  modules 的 100% line/branch gate。为接入这些 suites，production-module delta 只允许
  semantics-preserving impossible-branch simplification 与 `#[cfg(test)]` fault seams，不改变 public
  API 或 production behavior。
- integration evidence：`tests/identity_error_paths.rs`、
  `tests/keyed_incremental_error_paths.rs`、`tests/keyed_incremental_identity.rs`、
  `tests/measurement_identity.rs`。No unchanged `src/runtime/context_tests.rs` ownership is claimed。
- exact-head coverage：`.github/scripts/check_gh59_coverage.py`、
  `.github/scripts/gh59_coverage_lib.py`、`.github/scripts/gh59_coverage_source.py`、
  `.github/scripts/test_check_gh59_coverage.py`、
  `.github/scripts/test_check_gh59_coverage_security.py`。The source helper owns exact-HEAD Rust
  lexical/cfg classification and coverage-control rejection；the coverage helper owns fail-closed
  provenance/artifact parsing and production-only metrics；the CLI owns collect/produce/validate
  publication；both fixture suites own positive/negative and security bypass evidence。All
  collector commands directly execute the resolved and hashed `cargo-llvm-cov` launcher with its
  required `llvm-cov` argv sentinel and pinned nightly Cargo/rustc，不经过 Cargo alias dispatch。
- spec alignment：`specs/GH59/product.md`、`specs/GH59/tech.md`、
  `specs/GH59/tasks.md`。

`src/reconciler/identity.rs`、`src/reconciler/plan.rs`、
`src/layout/engine/incremental.rs` 与 `src/layout/engine/incremental_order.rs` remain the four
critical modules with 100% production line/branch gates；all compiler-observed production changed
executable lines retain the >=80% gate。Inline/external `#[cfg(test)]` code 不得进入 production
分母或分子；explicit `coverage(off)`、coverage-only cfg/cfg_attr、
`automatically_derived`/`naked` suppression 与 coverage-time build/config indirection 必须
fail closed。普通 declaration/comment 与当前 production target 不启用的非 coverage cfg
不伪装成 executable line。Because an unexpanded macro token stream cannot prove whether a
control-shaped token is an expression or forwarded meta，any non-literal `coverage`、
`coverage_nightly`、`automatically_derived` or `naked` token，以及 production-active macro
token stream 中任何 `path` or production-excluded module identifier，fails closed；tracked declarative
macro 中动态 attribute name synthesis（`#[$meta ...]`）也 fails closed；
same-named declarations/calls outside macro invocations remain allowed。Any non-literal/comment
`include` source token is forbidden，including import/re-export aliases and `include!`；`#[path]`
may name only a relative, non-parent-traversing `.rs` source；default external-module resolution、
the enclosing inline-module path and explicit `#[path]` are checked together。Any resolved target
whose stem/path component would be excluded from production metrics is permitted only when its
enclosing item is proven test-only by the same cfg span classifier；POSIX/Windows absolute targets
and every non-regular tracked HEAD tree entry are rejected before source filtering；normal untracked
sources fail the clean-worktree gate，and ignored `.rs` beneath production source roots fail an
explicit Git enumeration gate，so every production
compiled source blob remains in the tracked `.rs` scan。Stable full gates reject unstable local
token synthesis；output generated by already-resolved trusted dependency/procedural macros without
a complete control token in tracked source is part of the trusted expansion boundary and is not
independently attested by this lexical receipt。

Historical sequencing required GH-59 to rebase after GH-58 changed `src/layout/engine.rs`。
The current corrective base already contains that merged work；all listed engine edits must retain
its TextFlow cache、frame context 与 `try_compute*` cause chain，不得恢复旧 measurement 算法
或新增平行 engine。
...
- `runtime/context.rs` 仅为 scoped measurement candidate cardinality 与成功 publication
  replacement 修改；`hooks/use_measure.rs` 仅补 ambiguity 文档。`renderer/pipeline.rs` 负责
  checked identity/lookup error propagation及成功后发布完整 measurement snapshot，
  `app.rs` 仍只接 checked variant。TextFlow-only error 类型不加 identity variant。
- 不修改 GH-58 `text_flow.rs`、renderer output/projection、chat component 或 examples。
- 不在本 issue 重构 raw `Patch` topology，也不实现 apply rollback、一次 full rebuild、
  fallback、whole-frame terminal transaction 或 missing-layout renderer error；这些在 GH-60。
  GH-59 保证输入 validation 在 mutation 前完成，并且只有 exact order、target-exact map-set
  postcondition 与成功 renderer publication 都成立的 measurement snapshot 才能替换 runtime state。
...
| B-014 | public/runtime compatibility | `cargo check --workspace --all-targets --all-features --locked`；`cargo test --test keyed_incremental_identity --locked public_node_key_and_patch_surface_compiles -- --exact`；`cargo test --workspace --lib --locked runtime::context_tests::test_runtime_context_measurements_by_key -- --exact`；`cargo test --workspace --lib --locked runtime::context_tests::test_runtime_context_measurements_by_node_key_and_alias -- --exact`；`cargo test --workspace --lib --locked hooks::use_measure::tests::test_measure_element_by_key_with_runtime -- --exact` |
| B-015 | exact-head quality gates | full fmt/check/clippy/test；`cargo test --workspace --lib --locked reconciler::plan::tests::planned_nodes_are_shallow_while_patch_payloads_retain_full_subtrees -- --exact`；执行 `Exact-head coverage evidence` 的 trusted collect/produce/validate commands；fresh CI/reviewThreads/PR gate |
...
| B-019 | independent checked error chain / failed publication | `cargo test --test identity_error_paths --locked duplicate_incremental_target_changes_no_engine_state -- --exact`；`cargo test --test keyed_incremental_error_paths --locked duplicate_key_reaches_checked_layout_boundary -- --exact`；`cargo test --workspace --lib --locked renderer::pipeline::tests::identity_error_commits_no_frame_or_previous_vnode -- --exact`；`cargo test --workspace --lib --locked renderer::app::tests::duplicate_key_reaches_app_io_error_without_frame_commit -- --exact` |
| B-020 | engine composite + runtime 0/1/N lookup/publication | `cargo test --workspace --lib --locked layout::engine::tests::same_raw_key_across_parents_has_two_composite_layouts -- --exact`；`cargo test --workspace --lib --locked layout::engine::tests::raw_legacy_lookup_reports_typed_ambiguity -- --exact`；`cargo test --test measurement_identity --locked same_raw_key_in_distinct_parents_keeps_both_layout_entries -- --exact`；`cargo test --test measurement_identity --locked raw_legacy_lookup_fails_loudly_when_multiple_scopes_match -- --exact`；`cargo test --test measurement_identity --locked checked_raw_lookup_reports_the_match_count -- --exact`；`cargo test --workspace --lib --locked renderer::pipeline::tests::repeated_raw_measurement_key_is_typed_ambiguity -- --exact`；`cargo test --workspace --lib --locked renderer::pipeline::tests::measurement_candidates_transition_unique_ambiguous_unique -- --exact`（同时断言 alias checked/legacy ambiguity 与 stale composite 清理）；`cargo test --workspace --lib --locked hooks::use_measure::tests::measure_element_by_key_panics_on_ambiguous_runtime_alias_without_leaking_runtime -- --exact` |
...
| B-022 | coverage/property evidence | `cargo test --workspace --lib --locked reconciler::diff::tests::property_mixed_key_permutations_are_bijective_or_typed_error -- --exact`；运行 `Exact-head coverage evidence` 的 literal commands 并断言 validate 输出 `decision=allowed` |
| B-023 | checked public diff 与 fail-loud compatibility adapters | `cargo test --workspace --lib --locked reconciler::diff::tests::try_diff_invalid_nested_metadata_returns_error_without_partial_patches -- --exact`；`cargo test --workspace --lib --locked reconciler::diff::tests::try_diff_children_duplicate_returns_error_without_patches -- --exact`；`cargo test --workspace --lib --locked reconciler::diff::tests::legacy_diff_fails_loudly_on_invalid_input -- --exact`；`cargo test --workspace --lib --locked reconciler::diff::tests::legacy_diff_children_fails_loudly_without_mutating_destination -- --exact`；`cargo test --test keyed_incremental_error_paths --locked direct_create_duplicate_subtree_is_an_identity_rejection -- --exact` |
...
每条 filtered command 都必须先在相同 target/options 下用 libtest `--list --exact` 证明恰好
匹配 1 个测试；0 个或多个匹配不得算通过，也不得移除 `--exact` 扩大过滤范围。

## Exact-head coverage evidence

checker requires an absolute canonical repo root、40-character exact SHAs、a clean tracked
worktree、an existing output parent、a non-existing output directory and a remote-tracking trusted
base ref。Use the following literal canonical GH-59 flow；`origin/main` must resolve to the supplied base，
and the single merge-base must equal the supplied merge-base：

```bash
GH59_REPO_ROOT="$(git rev-parse --show-toplevel)"
GH59_TRUSTED_BASE_REF="origin/main"
GH59_HEAD="$(git -C "$GH59_REPO_ROOT" rev-parse HEAD)"
GH59_BASE="$(git -C "$GH59_REPO_ROOT" rev-parse --verify "refs/remotes/origin/main^{commit}")"
GH59_MERGE_BASE="$(git -C "$GH59_REPO_ROOT" merge-base "$GH59_HEAD" "$GH59_BASE")"
GH59_COVERAGE_PARENT="$GH59_REPO_ROOT/target/specrail/GH59"
GH59_COVERAGE_DIR="$GH59_COVERAGE_PARENT/coverage-$GH59_HEAD"
mkdir -p "$GH59_COVERAGE_PARENT"
test ! -e "$GH59_COVERAGE_DIR"
export PYTHONDONTWRITEBYTECODE=1

python3 "$GH59_REPO_ROOT/.github/scripts/check_gh59_coverage.py" collect \
  --repo-root "$GH59_REPO_ROOT" \
  --head "$GH59_HEAD" \
  --base "$GH59_BASE" \
  --merge-base "$GH59_MERGE_BASE" \
  --trusted-base-ref "$GH59_TRUSTED_BASE_REF" \
  --output-dir "$GH59_COVERAGE_DIR"

python3 "$GH59_REPO_ROOT/.github/scripts/check_gh59_coverage.py" produce \
  --repo-root "$GH59_REPO_ROOT" \
  --head "$GH59_HEAD" \
  --base "$GH59_BASE" \
  --merge-base "$GH59_MERGE_BASE" \
  --trusted-base-ref "$GH59_TRUSTED_BASE_REF" \
  --llvm-json "$GH59_COVERAGE_DIR/llvm-cov.json" \
  --cobertura "$GH59_COVERAGE_DIR/cobertura.xml" \
  --receipt "$GH59_COVERAGE_DIR/collection-receipt.json" \
  --evidence "$GH59_COVERAGE_DIR/coverage-evidence.json"

python3 "$GH59_REPO_ROOT/.github/scripts/check_gh59_coverage.py" validate \
  --repo-root "$GH59_REPO_ROOT" \
  --head "$GH59_HEAD" \
  --base "$GH59_BASE" \
  --merge-base "$GH59_MERGE_BASE" \
...
`collect` produces fixed-name `llvm-cov.json`、`cobertura.xml` and
`collection-receipt.json` in one directory；`produce` creates the non-preexisting
`coverage-evidence.json`；`validate` independently recomputes provenance and must print
`GH59 coverage evidence validated: decision=allowed`。The checker enforces changed executable
production lines >=80% and nonzero detailed production line/branch observations at 100% for all
four critical modules；test-only observations cannot pad either metric，coverage-control source or
collector indirection、missing/mismatched artifacts、dirty exact head、SHA/ref drift or any failed
phase is nonzero。The resolved coverage plugin、nightly Cargo/rustc and their sysroot LLVM tools、
Git/Python/OS/runner form the trusted collection infrastructure；the receipt binds only the recorded
plugin/Cargo/rustc/Git entrypoints and coverage artifacts，not vendor authenticity or unrecorded
infrastructure without an external signature or known-good digest。
...
old/new VNode trees
  ...
  -> engine atomically refreshes target-exact scoped/ElementId/composite compatibility maps
  -> assert removed subtree/old scope absent and map identity set equals target tree
  -> engine exposes validated scoped layouts + composite layouts + raw candidates
  -> renderer completes compatibility projection and render
       |-> failure: publish no runtime measurements and preserve previous snapshot
       |-> success: RuntimeContext atomically replaces ElementId/scoped/composite/raw/string views
             -> checked 0/1/N runtime lookup or fail-loud legacy measurement wrapper
...
- Compatibility：`NodeKey`/`Patch` 已公开。通过内部 plan 而非改公开字段布局，保留源码
  ...
  caller 使用 try variant，不能继续接受旧 HashMap 覆盖。runtime measurement helper 保持
  签名，唯一 alias 仍返回 exact scoped value；只有多 parent 候选由 checked API 返回 typed
  ambiguity、legacy helper fail loudly。成功 snapshot replacement 必须清除 stale aliases。
- Performance：递归 scoped identity 若反复 clone 完整路径会退化。实现应使用 arena/index
  ...
  node 重建 O(depth) String path。`PlannedNode` 只保存不含 children 的浅 VNode snapshot，
  递归结构只由 `PlannedNode.children` 表达；互不重叠的 Create/Replace patch payload 各自
  最多保留一次所需完整 subtree，禁止每层重复 clone 后代。性能阈值与 benchmark 在 GH-61。
...
- [ ] Runtime measurement：engine composite projection 仍保留两个 parent scope；pipeline exact
      tests 覆盖重复 raw/string typed ambiguity、legacy fail-loud 与 unique -> ambiguous ->
      unique 的 ElementId/scoped/composite/raw/string stale-state 清理；identity 失败不发布
      measurement snapshot。
      runtime context 与 hook exact tests 同时锁定既有唯一 key/raw/alias helper compatibility。
...
- [ ] Coverage：执行 `Exact-head coverage evidence` 的 literal Cobertura + fail-closed flow；changed executable
...
所有 filtered unit/integration gate 在正式执行前必须以相同 target/options 的
`--list --exact` 检查匹配数恰好为 1；该检查不替代任何 coverage 或 full gate。
...
raw `Patch` topology、whole-frame terminal transaction、rollback/rebuild/fallback 的回滚语义
同样保持 GH-60 范围；不得借 measurement compatibility 修改削弱任一现有 gate。
