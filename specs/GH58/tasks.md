...
- [ ] `SP58-T2`（lane alias: `GH58-T2`）实现 logical TextFlow core 与兼容 measure helper。Owner: `text-flow-core-lane` | Done when: `src/layout/text_flow.rs` 完整产生 immutable rows/positioned safe styled runs/logical bidirectional map/dispositions/diagnostics；hard-break/tab 先结构化消费，ESC/C0/DEL/C1 生成带原 range 的 safe replacement；`src/layout/mod.rs` 暴露必要类型；`src/layout/measure.rs` 委托同一算法；`tests/property_tests.rs` 由本 lane 独占并增加 logical map properties | Verify: `verify_lib_exact layout::text_flow::tests::text_flow_hard_breaks`; `verify_lib_exact layout::text_flow::tests::text_flow_control_replacement`; `verify_integration_exact property_tests text_flow_source_cell_round_trip`。
...
- [ ] `SP58-T5`（lane alias: `GH58-T5`）收敛 renderer、frame-local projection、render-to-string 与 public typed surface。Owner: `render-parity-lane` | Done when: tree/element renderer 只消费 logical runs并以当前 overflow/scroll/content rect/完整 clip stack/terminal bounds 新建不缓存的 RenderProjection；height/scroll/clip 改变不产生 stale visibility；dynamic pipeline 不提交 partial frame；`TextRenderError`、`try_render_to_string*` 与 exports 完成；本 lane 独占 `tests/text_flow_parity.rs`、`tests/prelude_surfaces.rs` | Verify: `verify_integration_exact text_flow_parity measure_rows_equal_rendered_rows`; `verify_integration_exact text_flow_parity viewport_projection_tracks_overflow_scroll_and_clip`; `verify_integration_exact text_flow_parity resize_reflows_or_reprojects_before_render`; `verify_integration_exact text_flow_parity source_controls_are_not_terminal_sequences`; `verify_integration_exact prelude_surfaces try_render_to_string_surface`。
...
- [ ] `SP58-T8`（lane alias: `GH58-T8`）贯通 App、static、terminal 与 test renderer failure boundary。Owner: `render-error-lane` | Done when: static content、App、TerminalController 与 TestRenderer 传播 T5 的同一 `TextRenderError` cause；App 保留 `io::Error` source；失败不提交 partial terminal/static output；本 lane 独占 `tests/text_flow_error_paths.rs`，不修改 parity/root-cause/source/property/prelude fixtures | Verify: `verify_integration_exact text_flow_error_paths typed_error_reaches_all_render_entrypoints`; `verify_integration_exact text_flow_error_paths failure_commits_no_partial_output`。
