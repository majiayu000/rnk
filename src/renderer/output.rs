...
use crate::layout::text_flow::sanitize_control;
...
impl Output {
    ...
    fn write_char_at(
        ...
        // Trust boundary: nothing that reaches a cell may be a control scalar,
        // however it got here. `flow_text` already substitutes them for the
        // text path, but `Output` is public and callers can hand us raw source.
        // A stored control would be emitted verbatim by the encoder and
        // executed by the terminal.
        let ch = sanitize_control(ch).unwrap_or(ch);
