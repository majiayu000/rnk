...
mod tests {
    ...
    #[test] fn gh62_revisioned_atomic_mutations() { use crate::components::chat::ConversationState; use std::num::NonZeroUsize; let mut state = ConversationState::new(0, NonZeroUsize::new(2).unwrap()); let message = ChatMessage::new(MessageId::new(1), ChatRole::Assistant, vec![MessageBlockEntry::new(BlockId::new(1), MessageBlock::Text(String::new()))]).unwrap(); state.apply_event(ConversationEvent::new(UpdateId::new("push").unwrap(), 0, ConversationUpdate::push(ConversationGuard::new(state.revision()), message))).unwrap(); let guard = MessageMutationGuard::new(ConversationGuard::new(state.revision()), MessageId::new(1), state.message(MessageId::new(1)).unwrap().revision()); let outcome = state.apply_event(ConversationEvent::new(UpdateId::new("append").unwrap(), 1, ConversationUpdate::append_text(guard, BlockId::new(1), "x").unwrap())).unwrap(); assert_eq!(state.message(MessageId::new(1)).unwrap().revision().get(), 2); assert_eq!(outcome.affected_messages()[0].previous_revision(), Some(MessageRevision::INITIAL)); }
    #[test] fn gh62_message_transition_matrix() { use crate::components::chat::state::valid_message_transition; let cause = FailureCause::new("x").unwrap(); let values = [MessageStatus::Pending, MessageStatus::Streaming, MessageStatus::Complete, MessageStatus::Cancelled, MessageStatus::Failed(cause)]; for from in 0..5 { for to in 0..5 { let expected = matches!((from, to), (0, 1) | (1, 2) | (0 | 1, 3 | 4)); assert_eq!(valid_message_transition(&values[from], &values[to], false), expected); }} }
    #[test] fn gh62_event_idempotency_contract() { use crate::components::chat::ConversationState; use std::num::NonZeroUsize; let mut state = ConversationState::new(0, NonZeroUsize::MIN); let message = ChatMessage::new(MessageId::new(1), ChatRole::User, vec![MessageBlockEntry::new(BlockId::new(1), MessageBlock::Text("x".into()))]).unwrap(); let event = ConversationEvent::new(UpdateId::new("same").unwrap(), 0, ConversationUpdate::push(ConversationGuard::new(state.revision()), message)); let first = state.apply_event(event.clone()).unwrap(); let before = state.clone(); assert_eq!(state.apply_event(event).unwrap(), first); assert_eq!(state, before); }
    #[test] fn gh62_replay_retention_boundary() { use crate::components::chat::ConversationState; use std::num::NonZeroUsize; let mut state = ConversationState::new(0, NonZeroUsize::MIN); for (sequence, id) in [(0, 1), (1, 2)] { let message = ChatMessage::new(MessageId::new(id), ChatRole::User, vec![MessageBlockEntry::new(BlockId::new(id), MessageBlock::Text("x".into()))]).unwrap(); let event = ConversationEvent::new(UpdateId::new(format!("e{id}")).unwrap(), sequence, ConversationUpdate::push(ConversationGuard::new(state.revision()), message)); state.apply_event(event).unwrap(); } assert_eq!(state.evicted_through(), Some(0)); assert_eq!(state.snapshot().retention().records()[0].event().sequence(), 1); }
    ...
    #[test] fn gh62_terminal_revision_race_contract() { use crate::components::chat::{ConversationError, ConversationState}; use std::num::NonZeroUsize; let mut state = ConversationState::new(0, NonZeroUsize::new(2).unwrap()); let message = ChatMessage::new(MessageId::new(1), ChatRole::User, vec![MessageBlockEntry::new(BlockId::new(1), MessageBlock::Text("x".into()))]).unwrap(); state.apply_event(ConversationEvent::new(UpdateId::new("push").unwrap(), 0, ConversationUpdate::push(ConversationGuard::new(state.revision()), message))).unwrap(); let first = MessageMutationGuard::new(ConversationGuard::new(state.revision()), MessageId::new(1), MessageRevision::INITIAL); state.apply_event(ConversationEvent::new(UpdateId::new("complete").unwrap(), 1, ConversationUpdate::complete(first))).unwrap(); let stale = MessageMutationGuard::new(ConversationGuard::new(state.revision()), MessageId::new(1), MessageRevision::INITIAL); let before = state.clone(); assert!(matches!(state.apply_event(ConversationEvent::new(UpdateId::new("race").unwrap(), 2, ConversationUpdate::cancel(stale))), Err(ConversationError::MessageRevisionMismatch { .. }))); assert_eq!(state, before); }
    #[test] fn gh62_cancellation_contract() { use crate::components::chat::ConversationState; use std::num::NonZeroUsize; let mut state = ConversationState::new(0, NonZeroUsize::new(2).unwrap()); let thinking = ThinkingContent::new(ThinkingId::new("t").unwrap(), "partial"); let message = ChatMessage::new(MessageId::new(1), ChatRole::Assistant, vec![MessageBlockEntry::new(BlockId::new(1), MessageBlock::Thinking(thinking))]).unwrap(); state.apply_event(ConversationEvent::new(UpdateId::new("push").unwrap(), 0, ConversationUpdate::push(ConversationGuard::new(state.revision()), message))).unwrap(); let guard = MessageMutationGuard::new(ConversationGuard::new(state.revision()), MessageId::new(1), MessageRevision::INITIAL); state.apply_event(ConversationEvent::new(UpdateId::new("cancel").unwrap(), 1, ConversationUpdate::cancel(guard))).unwrap(); let message = state.message(MessageId::new(1)).unwrap(); assert!(matches!(message.status(), MessageStatus::Cancelled)); assert!(matches!(message.blocks()[0].block(), MessageBlock::Thinking(value) if matches!(value.status(), ThinkingStatus::Cancelled) && value.content() == "partial")); }
}
